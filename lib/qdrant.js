...
export const TW_FOOD_COLLECTION = "tw_food";
...
export async function searchTwFood(query, limit = 5) {
  const vector = await embed(query);

  const results = await qdrant.search(TW_FOOD_COLLECTION, {
    vector,
    limit,
    with_payload: true,
  });
  ...
  return results.map((r) => ({
    score: r.score,
    food_id: r.payload.food_id,
    title: r.payload.title,
    type: r.payload.type,
    city: r.payload.city,
    famous_shop: r.payload.famous_shop,
